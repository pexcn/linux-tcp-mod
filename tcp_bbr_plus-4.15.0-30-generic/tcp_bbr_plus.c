...
struct bbr {
	...
	u64 ack_epoch_mstamp;	/* For tracking ACK aggregation */
	u16 extra_acked[2];	/* start of ACK sampling epoch */
...
#define ICSK_CA_PRIV_SIZE_FOR_BBR_PLUS (14 * sizeof(u64))
...
static void bbr_drain_to_target_cycling(struct sock *sk, const struct rate_sample *rs)
{
	struct tcp_sock *tp = tcp_sk(sk);
	struct bbr *bbr = inet_csk_ca(sk);
	u32 elapsed_us = tcp_stamp_us_delta(tp->delivered_mstamp, bbr->cycle_mstamp);
	u32 inflight, bw;
...
{
	BUILD_BUG_ON(sizeof(struct bbr) > ICSK_CA_PRIV_SIZE_FOR_BBR_PLUS);
